                        ...
                        <td>
                            <div style="display: flex; flex-direction: column; gap: 0.2rem;">
                                @if($stock->reference_id)
                                    <span style="font-weight: 600; font-size: 0.82rem;"><span style="color: #94a3b8;">Réf:</span> {{ $stock->reference_id }}</span>
                                @endif
                                @if($stock->notes)
                                    <span style="font-size: 0.78rem; color: #64748b; max-width: 200px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="{{ $stock->notes }}">{{ $stock->notes }}</span>
                                @endif
                            </div>
                        </td>
                        <td>
                            <div class="action-buttons justify-content-center">
                                <a href="{{ route('stock.edit', $stock->id) }}" class="btn-icon btn-icon-edit" title="Modifier">
                                    <i class="bi bi-pencil-fill"></i>
                                </a>
                                <form action="{{ route('stock.destroy', $stock->id) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer ce mouvement ?');" style="margin:0;">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="7" style="text-align: center; padding: 4rem 2rem; color: #94a3b8;">
                            ...
                            <h4 style="font-size: 1.1rem; font-weight: 700; color: #475569; margin-bottom: 0.4rem;">Aucun mouvement trouvé</h4>
