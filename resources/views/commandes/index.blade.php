...
@section('content')
    ...
    <style>
        ...
        .badge-statut.preparation {
            background: #fff7ed;
            color: #ea580c;
            border: 1px solid #fed7aa;
        }

        .badge-statut.expediee {
            background: #f5f3ff;
            color: #7c3aed;
            border: 1px solid #ddd6fe;
        }
        ...
        .badge-paiement.non-paye {
            background: #fef2f2;
            color: #ef4444;
            border: 1px solid #fecaca;
        }

        .badge-paiement.default {
            background: #f8fafc;
            color: #64748b;
            border: 1px solid #e2e8f0;
        }
    ...
    <div class="page-header">
        ...
        <div style="display: flex; gap: 0.8rem; align-items: center;">
            @if($showValidated)
                <a href="{{ route('commandes.index', array_merge(request()->all(), ['show_validated' => 0])) }}"
                    class="btn-back"
                    style="padding: 0.55rem 1rem; border-radius: 10px; text-decoration: none; display: inline-flex; align-items: center; gap: 0.4rem; font-size: 0.84rem; font-weight: 700; background: #f8fafc; border: 1.5px solid #e2eaf8; color: #64748b;">
                    <i class="bi bi-eye-slash-fill"></i> Masquer livrées
                </a>
            @else
                <a href="{{ route('commandes.index', array_merge(request()->all(), ['show_validated' => 1])) }}"
                    class="btn-back"
                    style="padding: 0.55rem 1rem; border-radius: 10px; text-decoration: none; display: inline-flex; align-items: center; gap: 0.4rem; font-size: 0.84rem; font-weight: 700; background: #eff6ff; border: 1.5px solid #bfdbfe; color: var(--blue-600);">
    ...
    <div class="stats-bar">
        ...
        <div class="stat-chip">
            ...
            <div>
                <div class="stat-chip-value">{{ number_format($stats['montant_total'], 2, ',', ' ') }} <span
                        style="font-size:0.72rem;font-weight:500;color:#64748b;">DH</span></div>
                    ...
                    <div class="filter-group">
                        ...
                        <div class="filter-input-wrapper">
                            ...
                            <input type="text" class="filter-input" name="numero_commande" id="numero_commande"
                                value="{{ request('numero_commande') }}" placeholder="Ex: CMD-2024-001...">
                        </div>
                    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <span style="font-weight:600;color:#1e293b; font-size: 0.8rem;">
                                    @if($commande->expedition)
                                        ...
                                        <span
                                            style="color:#64748b; font-size: 0.75rem;">{{ \Carbon\Carbon::parse($commande->expedition->date_expedition)->format('d/m/Y') }}</span>
                                    @else
                                        —
                                    @endif
                                </span>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <a href="{{ route('commandes.show', $commande->id) }}" class="btn-icon btn-icon-view"
                                        title="Détails">
                                        <i class="bi bi-eye-fill"></i>
                                    </a>
                                    @if($commande->statut !== 'Livrée' && $commande->statut !== 'Livré')
                                        <a href="{{ route('commandes.edit', $commande->id) }}" class="btn-icon btn-icon-edit"
                                            title="Modifier">
