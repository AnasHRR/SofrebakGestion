...
<div class="page-header">
    <h2><i class="bi bi-plus-circle-fill me-2" style="color:var(--blue-500);"></i>Nouvelle Commande Fournisseur</h2>
</div>
...
<div class="form-card">
    ...
    <form action="{{ route('commandesFournisseurs.store') }}" method="POST">
        @csrf
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label class="form-label">Date de la Commande</label>
                    <input type="date" name="date_commande" class="form-control" value="{{ old('date_commande', date('Y-m-d')) }}" required>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label class="form-label">Montant Total (DH)</label>
                    <input type="number" step="0.01" name="montant_total" class="form-control" value="{{ old('montant_total') }}" placeholder="Ex: 1500.00" required>
                </div>
            </div>
        </div>
                ...
                <div class="form-group">
                    ...
                    <select name="fournisseurs_id" class="form-control" required>
                        ...
                        @foreach($fournisseurs as $fournisseur)
                            <option value="{{ $fournisseur->id }}" {{ old('fournisseurs_id') == $fournisseur->id ? 'selected' : '' }}>
                                {{ $fournisseur->nom }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="form-group">
            ...
            <textarea name="notes" class="form-control" rows="4" placeholder="Détails supplémentaires ou remarques...">{{ old('notes') }}</textarea>
        </div>

        <div style="display: flex; gap: 1rem; margin-top: 2rem;">
            <button type="submit" class="btn-submit"><i class="bi bi-save"></i> Enregistrer</button>
