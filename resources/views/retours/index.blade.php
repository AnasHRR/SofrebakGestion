                ...
                <tbody>
                    @forelse ($retours as $retour)
                        <tr>
                            ...
                            <td>
                                <span
                                    style="font-size: 0.75rem; font-weight: 700; color: var(--blue-700); background: var(--blue-50); padding: 0.2rem 0.5rem; border-radius: 6px;">
                                    {{ $retour->region->nom ?? $retour->region_id }}
                                </span>
                            </td>
                            <td>
                                <div class="action-buttons justify-content-center">
                                    <a href="{{ route('retours.show', $retour->id) }}" class="btn-icon btn-icon-view"
                                        title="Détails">
                                        <i class="bi bi-eye-fill"></i>
                                    </a>
                                    <a href="{{ route('retours.edit', $retour->id) }}" class="btn-icon btn-icon-edit"
                                        title="Modifier">
                                        <i class="bi bi-pencil-fill"></i>
                                    </a>
                                    <form action="{{ route('retours.destroy', $retour->id) }}" method="POST"
                                        onsubmit="return confirm('Voulez-vous vraiment supprimer ce retour ?');"
                                        style="margin:0;">
                                        @csrf
                                        @method('DELETE')
