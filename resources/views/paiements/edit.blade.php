...
@section('title', 'Modifier Paiement - Sofrebak')
...
@section('content')
    ...
    <div class="page-header">
        <div class="page-header-left">
            <h2><i class="bi bi-pencil-fill me-2" style="color:var(--blue-500);"></i>Modifier Paiement #{{ $paiement->id }}</h2>
            <p>Mise à jour des informations du paiement</p>
        </div>
        <a href="{{ route('paiements.index') }}" class="btn-back">
                    ...
                    <div class="row g-4">
                            ...
                            <div class="input-group">
                                ...
                                <select name="client_id" class="form-select @error('client_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($clients as $client)
                                        <option value="{{ $client->id }}" {{ (old('client_id', $paiement->client_id) == $client->id) ? 'selected' : '' }}>{{ $client->nom_entreprise }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="montant" step="0.01" min="0" class="form-control @error('montant') is-invalid @enderror" value="{{ old('montant', $paiement->montant) }}" required>
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="date" name="date_paiement" class="form-control @error('date_paiement') is-invalid @enderror" value="{{ old('date_paiement', $paiement->date_paiement) }}" required>
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <select name="mode_paiement" id="mode_paiement" class="form-select @error('mode_paiement') is-invalid @enderror" required>
                                    <option value="" disabled>Choisir le mode</option>
                                    @foreach(['Espèces', 'Chèque', 'Virement', 'Carte bancaire'] as $mode)
                                        <option value="{{ $mode }}" {{ (old('mode_paiement', $paiement->mode_paiement) == $mode) ? 'selected' : '' }}>{{ $mode }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-6" id="cheque_field" style="display: none;">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="numero_cheque" id="numero_cheque" class="form-control @error('numero_cheque') is-invalid @enderror" placeholder="Entrez le numéro du chèque" value="{{ old('numero_cheque', $paiement->numero_cheque) }}">
                            </div>
                        ...
                        <div class="col-12 mt-5">
                            <div class="section-title">
                                <i class="bi bi-building"></i> Responsable & Localisation
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <select name="comptable_id" class="form-select @error('comptable_id') is-invalid @enderror" required>
                                    @foreach($comptables as $emp)
                                        <option value="{{ $emp->id }}" {{ (old('comptable_id', $paiement->comptable_id) == $emp->id) ? 'selected' : '' }}>{{ $emp->nom_complet }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="input-group">
                                ...
                                <select name="region_id" class="form-select @error('region_id') is-invalid @enderror" required>
                                    @foreach($regions as $reg)
                                        <option value="{{ $reg->id }}" {{ (old('region_id', $paiement->region_id) == $reg->id) ? 'selected' : '' }}>{{ $reg->nom }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3" required>{{ old('notes', $paiement->notes) }}</textarea>
                            @error('notes') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-12 mt-4 text-end">
                            <a href="{{ route('paiements.index') }}" class="btn btn-light me-2 fw-bold px-4 border">Annuler</a>
