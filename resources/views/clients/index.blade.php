...
<div class="page-header">
    <div class="page-header-left">
        <h2><i class="bi bi-people-fill me-2" style="color:var(--blue-500);"></i>Clients</h2>
        <p>Gestion et suivi de tous vos clients</p>
    </div>
    
    <div style="display: flex; gap: 1rem; align-items: center;">
        <form action="{{ route('clients.index') }}" method="GET" style="display: flex; gap: 0.5rem; margin: 0;">
            ...
            @if(request('search'))
                <a href="{{ route('clients.index') }}" class="btn-add" style="padding: 0.55rem 0.8rem; background: #fff5f5; color: #ef4444; border: 1px solid #fecaca; box-shadow: none; margin: 0;">
...
<div class="stats-bar">
    <div class="stat-chip">
        <div class="stat-chip-icon blue"><i class="bi bi-people-fill"></i></div>
        <div>
            <div class="stat-chip-value">{{ $total_clients }}</div>
            <div class="stat-chip-label">Clients au total</div>
        </div>
    </div>
    <div class="stat-chip">
...
<div class="table-card">
        ...
        <table class="custom-table">
            ...
            <tbody>
                @forelse ($clients as $cl)
                    <tr>
                        <td style="font-weight: 700;">{{ $cl->personne_contact }}</td>
                        <td>{{ $cl->nom_entreprise }}</td>
                        <td>
                            <a href="tel:{{ $cl->telephone }}" style="color: var(--blue-600); text-decoration: none;">
                                {{ $cl->telephone ?? '—' }}
                            </a>
                        </td>
                        <td>{{ $cl->adresse }}</td>
                        ...
                        <td>
                            <div class="action-buttons justify-content-center">
                                <a href="{{ route('clients.show', $cl->id) }}" class="btn-icon btn-icon-view" title="Détails">
                                    <i class="bi bi-eye-fill"></i>
                                </a>
                                <a href="{{ route('clients.edit', $cl->id) }}" class="btn-icon btn-icon-edit" title="Modifier">
                                    <i class="bi bi-pencil-fill"></i>
                                </a>
                                <form action="{{ route('clients.destroy', $cl->id) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer ce client ?');" style="margin:0;">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" style="text-align: center; padding: 4rem 2rem; color: #94a3b8;">
                            ...
                            <h4 style="font-size: 1.1rem; font-weight: 700; color: #475569; margin-bottom: 0.4rem;">Aucun client trouvé</h4>
                            <p style="font-size: 0.85rem;">Commencez par ajouter votre premier client.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="pagination-wrapper">
        <span style="font-size: 0.85rem; color: #64748b; font-weight: 600;">Total : {{ $total_clients }} client(s)</span>
        <div>
            {{ $clients->links() }}
        </div>
    </div>
